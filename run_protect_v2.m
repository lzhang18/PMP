function [mp_protected, mpi_protected,mp, mpi]=run_protect_v2(L,L_attack,ts,skip)
%Parameters for PMP:
...
for k=1:length(target_interval)
    ...
    disp(k)
    %check tail:
    for i=ends+1:min(length(mp),ends+P)
        if(skip(i)==1)
            continue;
        end
        d_mask = abs(MASS_V3(ts, ts(i:i+L)));
        mask_idx1_start = max(1,i-P);
        mask_idx1_end = min(length(mp),i+P);
        d_mask(mask_idx1_start:mask_idx1_end)=inf;
        idx_remove(1) = mpi(ends);
        mask_idx2_start = max(1,idx_remove(1)-P);
        mask_idx2_end = min(length(mp),idx_remove(1)+P);
    ...
    for i=start:ends
        ...
        d = abs(MASS_V3(ts, ts(i:i+L)));
        d(max(1,i-P):min(length(mp),i+P))=inf;
        d2 = d;
        if i-start+1<mask_len
            idx_remove(1) = target_idx;
            for l=1:length(idx_remove)
                d2(max(1,idx_remove(l)-P):min(length(mp),idx_remove(l)+P))=inf;
            end
        else
            target_idx=start;
            count = 0;
